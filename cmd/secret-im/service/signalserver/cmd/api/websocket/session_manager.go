...
// 创建Session管理器
func NewSessionManager(ctx *svc.ServiceContext, router http.Handler, makeSessionHandler MakeSessionHandler) *SessionManager {
	return &SessionManager{
		router:             router,
		pushSender:         ctx.PushSender,
		pubSubManager:      ctx.PubSubManager,
...
func (manager *SessionManager) HandleAccept(w http.ResponseWriter, r *http.Request) {
	...
	//这是一个回调函数，通知websocket连接来了，handler做一些事情
