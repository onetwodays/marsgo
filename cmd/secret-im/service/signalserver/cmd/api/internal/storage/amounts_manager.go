...
func (m AccountManager) Get (ambiguousIdentifier auth.AmbiguousIdentifier) (*entities.Account,error){
	...
	if len(ambiguousIdentifier.UUID)!=0{
		dbAccount,err=signal.SC.Svc.AccountsModel.FindOneByUuid(ambiguousIdentifier.UUID)
	}
	if dbAccount==nil{
		dbAccount,err = signal.SC.Svc.AccountsModel.FindOneByNumber(ambiguousIdentifier.Number)
	}
	if err!=nil{
		return nil, err
	}
...
func (m AccountManager) GetByNumber (number string) (*entities.Account,error){
	...
	dbAccount,err := signal.SC.Svc.AccountsModel.FindOneByNumber(number)
	if err!=nil {
		return nil, err
	}
	return DbAccount2AppAccount(dbAccount)
