...
var (
    ...
    blockchain   = []*Block{genesisBlock} //区块链
...
func handleMineBlock(w http.ResponseWriter, r *http.Request) {
    ...
    addBlock(block)
    resp:=responseLatestMsg()
    broadcast(resp) //把区块信息广播出去
...
func handleBlockchainResponse(msg []byte) {
    ...
    err := json.Unmarshal(msg, &receivedBlocks) //有可能传递的是一个块
...
func main() {
    ...
    http.HandleFunc("/blocks", handleBlocks)//查询全部区块数据
    http.HandleFunc("/mine_block", handleMineBlock)//新增一个区块数据
    http.HandleFunc("/peers", handlePeers) //查询所有的peer client
